...
void OnDataRecv(const uint8_t * mac,
  ...
  decrypt_string_with_TDES_AES_Blowfish_Serp(ciphertext); // Step 6
  ...
  if (packet_one == true && packet_two == true) {
    if (myData.b == false) {
      ...
      tft.setTextSize(1);
      tft.setCursor(0, 2);
      tft.print("Received string:");
      tft.setCursor(0, 14);
    }
    for (int i = 0; i < 14; i++) {
      if (int(dec_st.charAt(i)) < 127 && int(dec_st.charAt(i)) > 31)
        tft.print(dec_st.charAt(i));
    }
  }
  ...
  if (packet_one == true && packet_two == false) {
    ...
    String decr_tag;
    for (int i = 0; i < 6; i++) {
      decr_tag += string_with_data.charAt(i);
    }
    //Serial.println(decr_tag);
    if (decr_tag != prefix)
      prefix_dont_match();
    delay(24);
    modify_keys(); // Step 16
    ...
    tft.setTextColor(0xffff);
    tft.setTextSize(1);
    disp_centered_text("Waiting For Data", 2);
    disp_centered_text("From The Receiver", 16);
  }

  if (packet_one == false && packet_two == false) {
    dsrnd = true;
    tft.fillScreen(0x0000);
    tft.setTextColor(0xffff);
    tft.setTextSize(1);
    disp_centered_text("Setting Up Keys", 2);
    disp_centered_text("Please Wait", 16);
    disp_centered_text("For A While", 30);
    string_with_data = dec_st;
    delay(24);
    modify_keys(); // Step 6
    delay(24);
    generate_first_random_value(); // Step 7
    delay(24);
    encrypt_and_send_packet(); // Steps 8, 9
    delay(24);
    modify_keys(); // Step 10
    packet_one = true;
  }
...
void integrity_verification_failed() {
  ...
  disp_centered_text("Verification", 44);
  disp_centered_text("Failed!!!", 68);
  while (1);
}

void prefix_dont_match() {
  dsrnd = true;
  delay(24);
  tft.fillScreen(0x0000);
  tft.setTextSize(2);
  tft.setTextColor(0xf800);
  disp_centered_text("Prefixes", 20);
  disp_centered_text("Don't Match!", 44);
  delay(500);
  while (1);
}
...
void setup() {
  ...
  disp_centered_text("Unlock The Transmitter", 115);

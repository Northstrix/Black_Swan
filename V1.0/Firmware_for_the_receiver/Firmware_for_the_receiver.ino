  ...
  */
  ...
  if (curr > fir && curr < (fir + 7500)){
    ...
    */
    ...
    display.println("Keys and IVs set up successfully.");
    display.display();    
  }
  else {
    ...
    display.setTextColor(BLACK);
    display.setCursor(0,0);
    display.println("Failed to set up secure communication channel. Reboot the device and try again.");
    display.display();    
  }
}
...
void wfip(){
  ...
  display.setTextColor(BLACK);
  display.setCursor(0,0);
  display.println("Waiting for the initialization package to arrive.");
  display.display();
}
